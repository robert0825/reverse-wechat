...
public class TXCDRApi
{
  ...
  {
    if (paramString != null) {
      nativeSetCommonValue(TXCDRDef.DR_KEY_APP_VER, paramString);
    }
  }
  ...
  {
    if (mDevType != null) {
      nativeSetCommonValue(TXCDRDef.DR_KEY_DEV_TYPE, mDevType);
    }
    if (mNetType != null) {
      nativeSetCommonValue(TXCDRDef.DR_KEY_NET_TYPE, mNetType);
    }
    if (mDevId != null) {
      nativeSetCommonValue(TXCDRDef.DR_KEY_DEV_ID, mDevId);
    }
    if (mDevUUID != null) {
      nativeSetCommonValue(TXCDRDef.DR_KEY_DEV_UUID, mDevUUID);
    }
    if (mAppName != null) {
      nativeSetCommonValue(TXCDRDef.DR_KEY_APP_NAME, mAppName);
    }
    if (mSysVersion != null) {
      nativeSetCommonValue(TXCDRDef.DR_KEY_SYS_VER, mSysVersion);
    }
    if (mMacAddr != null) {
      nativeSetCommonValue(TXCDRDef.DR_KEY_MAC_ADDR, mMacAddr);
    }
  }
...
/* Location:              D:\tools\apktool\weixin6519android1140\jar\classes2-dex2jar.jar!\com\tencent\liteav\basic\datareport\TXCDRApi.class
